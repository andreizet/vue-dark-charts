...
import type { ChartTheme, DonutSegment } from '../types'

type SegmentedCircleVariant = 'donut' | 'pie'

interface SegmentedCircleChartProps {
  segments: DonutSegment[]
  neon?: boolean
  theme?: ChartTheme
  centerText?: string
  format?: (value: number) => string
  variant?: SegmentedCircleVariant
}

const props = withDefaults(defineProps<SegmentedCircleChartProps>(), {
  theme: 'dark',
  neon: true,
  variant: 'donut',
})
...
function buildDonutArcPath(startDeg: number, endDeg: number): string {
...
function buildPieSlicePath(startDeg: number, endDeg: number): string {
  const sweep = endDeg - startDeg
  const outerCornerOffset = Math.min(
    sweep / 4,
    (CORNER_RADIUS / OUTER_RADIUS) * (180 / Math.PI),
  )
  const outerStart = polarToCartesian(OUTER_RADIUS, startDeg)
  const outerEnd = polarToCartesian(OUTER_RADIUS, endDeg)
  const outerArcStart = polarToCartesian(OUTER_RADIUS, startDeg + outerCornerOffset)
  const outerArcEnd = polarToCartesian(OUTER_RADIUS, endDeg - outerCornerOffset)
  const outerStartJoin = polarToCartesian(OUTER_RADIUS - CORNER_RADIUS, startDeg)
  const outerEndJoin = polarToCartesian(OUTER_RADIUS - CORNER_RADIUS, endDeg)
  const innerStart = polarToCartesian(PIE_CENTER_RADIUS, startDeg)
  const innerEnd = polarToCartesian(PIE_CENTER_RADIUS, endDeg)
  const largeArc = sweep > 180 ? 1 : 0

  return [
    `M ${innerStart.x} ${innerStart.y}`,
    `L ${outerStartJoin.x} ${outerStartJoin.y}`,
    ...
    `Q ${outerEnd.x} ${outerEnd.y} ${outerEndJoin.x} ${outerEndJoin.y}`,
    `L ${innerEnd.x} ${innerEnd.y}`,
    `A ${PIE_CENTER_RADIUS} ${PIE_CENTER_RADIUS} 0 ${largeArc} 0 ${innerStart.x} ${innerStart.y}`,
    'Z',
  ].join(' ')
}
...
function buildArcPath(startDeg: number, endDeg: number): string {
  if (props.variant === 'pie') {
    return buildPieSlicePath(startDeg, endDeg)
  }

  return buildDonutArcPath(startDeg, endDeg)
}
...
<template>
  <div ref="rootRef" class="vdc-root vdc-chart">
    ...
    <div v-else :class="['vdc-donut', { 'is-compact': compactLayout, 'is-pie': props.variant === 'pie' }]">
      <div class="vdc-donut-canvas" :style="{ width: `${chartSize}px` }">
        <svg :width="chartSize" :height="chartSize" :viewBox="`0 0 ${VIEWBOX_SIZE} ${VIEWBOX_SIZE}`">
          <defs>
            <filter
              v-for="arc in arcs"
              :id="`vdc-donut-glow-${props.variant}-${arc.index}`"
              :key="`glow-${arc.index}`"
              x="-50%"
              y="-50%"
              width="200%"
          ...
          <path
            ...
            :filter="props.neon ? `url(#vdc-donut-glow-${props.variant}-${arc.index})` : undefined"
            :transform="
          ...
          <template v-if="props.variant === 'donut'">
            <text
              :x="CX"
              :y="CY - 6"
              text-anchor="middle"
